...
static void handle_T__(const uint8_t *p, uint_fast32_t size, char *target, int targetsize)
{
	...
	temp_p[j++]=0;
	fprintf(stderr, "handle_T__(*p=(charset=%d;data=\"%s\"), size=%d, target=%p, targetsize=%d\n", p[0], temp_p, size, target, targetsize);
	free(temp_p);
#endif
	target[0]=0;
	if (size<1)
...
static void handle_T___(const uint8_t *p, uint_fast32_t size, char *target, int targetsize)
{
	...
	temp_p[j++]=0;
	fprintf(stderr, "handle_T___(*p=(charset=%d;data=\"%s\"), size=%d, target=%p, targetsize=%d) ", p[0], temp_p, size, target, targetsize);
	fprintf(stderr, "forwarding to handle_T__\n");
	free(temp_p);
#endif
...
static void parseid3v1(struct moduleinfostruct *m, const unsigned char *tag)
{
	...
	temp_tag[j++]=0;
	fprintf(stderr, "parseid3v1(m=%p, tag=%s) ", m, temp_tag);
#endif
	if (!memcmp(tag, "TAG", 3))
	{
		if (memcmp(tag+3, "                              ", 30))
...
#ifdef ID3_DEBUG
	fprintf(stderr, "unsync(src=%p, orgsize=%d) =", src, orgsize);
#endif
	if (!orgsize)
	{
#ifdef ID3_DEBUG
...
{
#ifdef ID3_DEBUG
	fprintf(stderr, "parseid3v20(m, id3v2header = %p (not used), id3v3data = %p, len = %d\n", id3v2header, id3v2data, len);
#endif
	while (1)
	{
		uint_fast32_t size;
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v20, iterate, len = %d\n", len);
#endif
		if (len<1)
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 len<1, return\n");
#endif
		...
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 padding reached, return\n");
#endif
		...
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 len<6, return\n");
#endif
			return 1; /* out of data */
		}
		size = (id3v2data[3]<<16)|(id3v2data[4]<<8)|(id3v2data[5]);
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v20 size=%06x (computed from id3v2data[3]=%02x id3v2data[4]=%02x id3v2data[5]=%02x)\n", size, id3v2data[3], id3v2data[4], id3v2data[5]);
#endif
		if (len<(size+6))
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 len<(size+6), return\n");
#endif
			return 1; /* out of data */
		}
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v20 tag=\"%c%c%c\"\n", id3v2data[0], id3v2data[1], id3v2data[2]);
#endif
		if (!memcmp(id3v2data, "TP1", 3))
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 store into m->composer (size=%d)\n", sizeof(m->composer));
#endif
			handle_T__(id3v2data+6, size, m->composer, sizeof(m->composer));
		} else if (!memcmp(id3v2data, "TT2", 3))
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v20 store into m->modname (size=%d)\n", sizeof(m->modname));
#endif
			handle_T__(id3v2data+6, size, m->modname, sizeof(m->modname));
		}
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v20 skipping 6+size into id3v2data/len\n");
#endif
		len-=(size+6);
		id3v2data+=(size+6);
	}
...
{
#ifdef ID3_DEBUG
	fprintf(stderr, "parseid3v23(m, id3v2header = %p (not used), id3v3data = %p, len = %d\n", id3v2header, id3v2data, len);
#endif
	while (1)
	{
		uint_fast32_t size;
...
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v23, iterate, len = %d\n", len);
#endif
		if (len<1)
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v23 len<1, return\n");
#endif
			return 1; /* out of data */
	 	}
		if (id3v2data[0]==0)
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v23 padding reached, return\n");
#endif
			return 0; /* padding reached */
		}
		if (len<10)
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v23 len<10, return\n");
#endif
			return 1; /* out of data */
		}
...
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v23 size=%08x (computed from id3v2data[4]=%02x id3v2data[5]=%02x id3v2data[6]=%02x id3v2data[7]=%02x)\n", size, id3v2data[4], id3v2data[5], id3v2data[6], id3v2data[7]);
#endif
		if (len<(size+10))
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v23 len<(size+10), return\n");
#endif	
			return 1; /* out of data */
		}
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v23 flags: id3v2data[8]=%02x id3v2data[9]=%02x\n", id3v2data[8], id3v2data[9]);
#endif
...
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v23 unsupported flag detected (like zlib compression etc.)\n");
#endif
		} else {
			int offset=10;
...
#ifdef ID3_DEBUG
				fprintf(stderr, "// parseid3v23 unsync bit set, size updated to %d\n", size);
#endif
			}
			if (id3v2data[9]&0x01)
			{
#ifdef ID3_DEBUG
				fprintf(stderr, "// parseid3v23 size-validation check, ignoring, skipping 4 bytes\n");
#endif
...
{
#ifdef ID3_DEBUG
	fprintf(stderr, "parseid3v2(m, id3v2header = %p, id3v3data = %p, len = %d\n", id3v2header, id3v2data, len);
	fprintf(stderr, "// parseid3v2 tag version 2.%d\n", id3v2header[3]);
#endif
...
#ifdef ID3_DEBUG
		fprintf(stderr, "// parseid3v2 global unsync bit set, new len = %d\n", len);
#endif
	}
	...
	if (id3v2header[3]<4)
	{
		...
		{
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v2 TAG version lower than 2.4, and extended tag flaged, trying to skip it (len is fixed to 10 bytes)\n");
#endif
			if (len<10)
			{
#ifdef ID3_DEBUG
				fprintf(stderr, "// parseid3v2 to little data to skip, return \n");
#endif
		...
		if (id3v2header[5]&0x40)
		{
			uint_fast32_t esize;
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v2 TAG version greater or equal to 2.4, and extended tag flaged, trying  to skip it\n");
#endif
			/*fprintf(stderr, "Removing extended header\n");*/
			if (len<6)
			{
#ifdef ID3_DEBUG
				fprintf(stderr, "// parseid3v2 to little data available to calculate extended header length (len<6), return\n");
#endif
				return 1; /* failed to parse */
			}
			esize = (id3v2data[0]<<21)|(id3v2data[1]<<14)|(id3v2data[2]<<7)|(id3v2data[3]);
#ifdef ID3_DEBUG
			fprintf(stderr, "// parseid3v2 extended header size=%08x, calculated from id3v2data[0]=%02x, id3v2data[1]=%02x, id3v2data[2]=%02x, id3v2data=%02x\n", esize, id3v2data[0], id3v2data[1], id3v2data[2], id3v2data[3]);
#endif
			if (len<esize)
			{
#ifdef ID3_DEBUG
				fprintf(stderr, "// parseid3v2 skipping extended header failed, to little data available (len<size), return\n");
#endif
				return 1; /* failed to parse */
			}
			len-=esize;
...
#ifdef ID3_DEBUG
	fprintf(stderr, "// parseid3v2, calling sub-parser based on version\n");
#endif
	if (id3v2header[3]<=2)
